...
export const Footer = () => {
  ...
  return (
    <footer className="bg-zinc-50 border-t border-zinc-200 pt-20 pb-10 text-left relative overflow-hidden">
      {/* Footer background glows */}
      <div className="absolute bottom-0 right-0 w-84 h-84 rounded-full bg-brand-red/5 blur-3xl -z-10" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-8 pb-16 border-b border-zinc-200">
          ...
          <div className="lg:col-span-4 flex flex-col gap-5">
            ...
            <p className="text-sm text-zinc-550 leading-relaxed max-w-sm font-medium">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            <h4 className="text-sm font-bold text-zinc-800 uppercase tracking-widest border-l-2 border-brand-red pl-2">
              Explore
            </h4>
            <ul className="flex flex-col gap-3">
              {quickLinks.map((link, idx) => (
                <li key={idx}>
                  <a
                    href={link.href}
                    className="text-sm text-zinc-650 hover:text-brand-red transition-colors duration-250 font-semibold"
                  >
                    {link.label}
                  </a>
                </li>
            ...
            <form onSubmit={handleSubscribe} className="flex gap-2 mt-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white border border-zinc-300 rounded-lg py-2.5 pl-9 pr-3 text-xs text-zinc-800 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red transition-all"
                />
              </div>
              <Button
                variant="primary"
                size="sm"
                type="submit"
                className="px-3 shrink-0 py-2.5 rounded-lg flex items-center justify-center cursor-pointer shadow-md shadow-brand-red/10"
              >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-10 text-xs text-zinc-550 font-semibold">
